...
export class HomeComponent implements OnInit {
  ...
  // handleBarChartData(event: MessageEvent) {
  //   try {
  //     const data: totalStationName[] = JSON.parse(event.data);
  //     const validData = data.filter(item => item.TotalRevenue > 0 || item.TotalProfit > 0);
  ...
  //     // Nếu biểu đồ không tự cập nhật thì ép cập nhật
  //     // this.chart?.update();
