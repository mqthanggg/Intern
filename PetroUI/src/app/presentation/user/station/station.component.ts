...
export class StationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      dispenser: this.http.get(environment.serverURI + `/dispenser/station/${this.id}?token=${localStorage.getItem('jwt')}`, { observe: "response", withCredentials: true }),
      tank: this.http.get(environment.serverURI + `/tank/station/${this.id}?token=${localStorage.getItem('jwt')}`, { observe: "response", withCredentials: true }),
    }).
    pipe(
